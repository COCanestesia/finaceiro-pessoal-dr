...
class TransferResult:transfer_id:int;source_entry_id:int;target_entry_id:int
class BankService:
    ...
    def balance(self,account_id:int,at_date:date|None=None)->int:
        ...
        regular=int(self.connection.execute("SELECT COALESCE(SUM(CASE WHEN entry_type='RECEITA' THEN amount_cents WHEN entry_type='DESPESA' THEN -amount_cents ELSE 0 END),0) FROM financial_entry WHERE bank_account_id=? AND deleted_at IS NULL AND status IN ('PAGO','RECEBIDO')"+date_sql,params).fetchone()[0])
        tparams=[account_id,account_id,account_id];tsql=''
        if at_date:tsql=' AND transfer_date<=?';tparams.append(at_date.isoformat())
        transfers=int(self.connection.execute('SELECT COALESCE(SUM(CASE WHEN target_account_id=? THEN amount_cents ELSE -amount_cents END),0) FROM internal_transfer WHERE (source_account_id=? OR target_account_id=?)'+tsql,tparams).fetchone()[0])
        cparams=[account_id];csql=''
        if at_date:csql=' AND paid_at<=?';cparams.append(at_date.isoformat())
        card_payments=int(self.connection.execute('SELECT COALESCE(SUM(amount_cents),0) FROM card_invoice_payment WHERE bank_account_id=?'+csql,cparams).fetchone()[0])
        return int(row[0])+regular+transfers-card_payments
    def transfer(self,source_id:int,target_id:int,amount_cents:int,transfer_date:date,description:str)->TransferResult:
        ...
        try:
            common=dict(description=description,amount_cents=amount_cents,entry_type='TRANSFERENCIA',status='PAGO',competence_date=transfer_date,settled_date=transfer_date,allow_duplicate=True);src=self.finance.create_entry(CreateEntry(**common,bank_account_id=source_id));dst=self.finance.create_entry(CreateEntry(**common,bank_account_id=target_id));cur=self.connection.execute('INSERT INTO internal_transfer(source_account_id,target_account_id,amount_cents,transfer_date,description,source_entry_id,target_entry_id) VALUES (?,?,?,?,?,?,?)',(source_id,target_id,amount_cents,transfer_date.isoformat(),description,src,dst));tid=int(cur.lastrowid)
            if owns:self.connection.commit()
            return TransferResult(tid,src,dst)
        except Exception:
