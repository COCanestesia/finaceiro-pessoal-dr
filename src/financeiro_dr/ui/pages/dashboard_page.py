...
from financeiro_dr.core.financeiro.dashboard_page_support import extra_dashboard_metrics
...
class DashboardPage(QWidget):
    ...
    def refresh(self):
        ...
        snap=self.service.snapshot(date.today());cards=[('Receitas do mês',format_money(snap.income_cents)),('Despesas do mês',format_money(snap.expense_cents)),('Resultado',format_money(snap.result_cents)),('Atrasadas',str(snap.overdue_count)),('Pagar hoje',format_money(snap.pay_today_cents)),('Amanhã',format_money(snap.tomorrow_cents)),('Próx. 7 dias',format_money(snap.next_7_days_cents)),('Próx. 30 dias',format_money(snap.next_30_days_cents))];alerts=[]
        if self.connection is not None:
            try:
                balance,invoice_open,pending=extra_dashboard_metrics(self.connection,date.today());nw=NetWorthService(self.connection).snapshot(date.today());cards=[('Saldo disponível',format_money(balance))]+cards+[('Faturas em aberto',format_money(invoice_open)),('Patrimônio',format_money(nw.assets_cents)),('Investimentos',format_money(nw.investments_cents)),('Obrigações',format_money(nw.obligations_cents)),('Patrimônio Líquido',format_money(nw.net_worth_cents)),('Conciliações pendentes',str(pending))];balerts=BudgetAlertService(BudgetService(self.connection)).for_month(date.today().year,date.today().month)
