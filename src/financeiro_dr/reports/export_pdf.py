...
def export_pdf(table:ReportTable,destination:Path)->Path:
    destination=Path(destination);destination.parent.mkdir(parents=True,exist_ok=True);pagesize=landscape(A4) if len(table.columns)>7 else A4;doc=SimpleDocTemplate(str(destination),pagesize=pagesize,rightMargin=24,leftMargin=24,topMargin=24,bottomMargin=24);styles=getSampleStyleSheet();story=[Paragraph(table.title,styles['Title']),Paragraph(table.filters_text or 'Relatório financeiro',styles['Normal']),Paragraph(f"Gerado em {datetime.now():%d/%m/%Y %H:%M}",styles['Normal']),Spacer(1,10)];data=[list(table.columns)]+[[str(v) for v in row] for row in table.rows]
    if table.total_cents is not None:data.append(['Total']+['']*max(0,len(table.columns)-2)+[f'R$ {table.total_cents/100:,.2f}'])
